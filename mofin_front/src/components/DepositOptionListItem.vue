<template>
  <div class="card option-item mx-2 my-4">
    <div class="card-body d-flex flex-column align-items-center">
      ...
      <p class="card-text px-2 pt-3">유형: {{ depositOption.intr_rate_type_nm }}</p>
      <p class="card-text px-2">저축 단위: {{ depositOption.save_trm }} 개월</p>
      <p class="card-text px-2">저축금리: {{ depositOption.intr_rate }} %</p>
      <p class="card-text px-2">최고 우대 금리: {{ depositOption.intr_rate2 }} %</p>
      <button v-if="accountsstore.isLogin" @click="addOrCancelProduct(depositOption.fin_prdt_cd, depositOption.save_trm)" class="btn btn-pastel-yellow mb-3">
        {{ isProductAdded(depositOption.fin_prdt_cd, depositOption.save_trm) ? '가입 취소 하기' : '가입하기' }}
      </button>
    </div>
  </div>
</template>
